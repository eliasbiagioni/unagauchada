...
<html>
    ...
    <div class="contenedorRegistroGauchada letra" >
        <?php 
            echo form_open('verGauchadaCompleta/mandarPregunta','name="formPreguntas" onsubmit="return noVacio()""'); 
            echo form_textarea($area_pregunta);
            echo '<span>'.form_error('pregunta').'</span>';
            echo br();
            ?>
            <input type="hidden" name="id_favor" value="<?php echo $id_favor ?>">
            <input type="hidden" name="id_usuario" value="<?php echo $id_usuario ?>">
            <?php
            echo form_submit('','Enviar pregunta');
            echo form_close();
        ?>
    </div>
    </body>
